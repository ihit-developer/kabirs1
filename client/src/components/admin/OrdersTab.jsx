...
const STATUS_META = {
  ...
  cancelled: {
    label: "Cancelled",
    dot: "bg-red",
    chip: "bg-red/15 text-red border-red/30",
  },
};
...
export default function OrdersTab() {
  ...
  const setEstimate = async (id) => {
    ...
    try {
      const updated = await api.put(`/orders/${id}/status`, {
        status: orders.find((o) => o._id === id).status,
  ...
  const setKitchenNote = async (id, current) => {
    ...
    if (note === null) return;
    try {
      const updated = await api.put(`/orders/${id}/kitchen-notes`, {
        kitchenNotes: note,
      });
      setOrders((prev) => prev.map((o) => (o._id === id ? updated : o)));
    } catch (err) {
  ...
  const verifyPayment = async (id) => {
    try {
      const updated = await api.put(`/orders/${id}/verify-payment`, {
      ...
      <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
        <div>
          ...
          <p className="text-white/50 text-sm flex items-center gap-2 mt-1">
            <span
              className={`w-2 h-2 rounded-full ${live ? "bg-emerald-400 animate-pulseSoft" : "bg-white/20"}`}
            />
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {visible.map((o) => {
            ...
            const isNew =
              o.status === "pending" &&
              Date.now() - new Date(o.createdAt).getTime() < 5 * 60000;
            ...
            return (
              <div
                ...
                className={`bg-bg2 border rounded-2xl overflow-hidden transition ${
                  isNew
                    ? "border-gold/50 shadow-[0_0_0_1px_rgba(240,193,75,.25)]"
                    : "border-white/10"
                }`}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold truncate">
                        {o.customerName}{" "}
                        {isNew && (
                          <span className="ml-1 text-[10px] font-bold bg-gold text-bg px-1.5 py-0.5 rounded-full align-middle">
                            NEW
                          </span>
                        )}
                      </p>
                      <p className="text-white/40 text-xs mt-0.5">
                        #{String(o._id).slice(-6).toUpperCase()} · {o.phone} ·{" "}
                        {timeAgo(o.createdAt)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span
                      className={`text-xs font-bold px-3 py-1.5 rounded-full border whitespace-nowrap ${meta.chip}`}
                    >
                      {meta.label}
                    </span>
                    <i
                      className={`fa-solid fa-chevron-down text-white/30 text-xs transition-transform ${isOpen ? "rotate-180" : ""}`}
                    />
                  </div>
                </button>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 mb-4 text-sm">
                      <div>
                        ...
                        <p className="text-white/80">
                          {o.address || o.orderType} · {o.branch} branch
                        </p>
                      </div>
                      <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {o.paymentMethod !== "cod" && !o.paymentVerified && (
                        <button
                          ...
                          className="text-xs font-semibold text-emerald-400 border border-emerald-400/30 px-3 py-1.5 rounded-full hover:bg-emerald-400/10"
                        >
                          <i className="fa-solid fa-check mr-1" /> Verify
                          Payment
                        </button>
                      )}
                      <button
                      ...
                      <button
                        onClick={() => setKitchenNote(o._id, o.kitchenNotes)}
                        className="text-xs font-semibold text-white/60 border border-white/15 px-3 py-1.5 rounded-full hover:bg-white/5"
                      >
                        <i className="fa-solid fa-note-sticky mr-1" /> Note
                      </button>
                      <button
                        onClick={() => setEstimate(o._id)}
                        className="text-xs font-semibold text-white/60 border border-white/15 px-3 py-1.5 rounded-full hover:bg-white/5"
                      >
                      ...
                      <button
                        ...
                        className="text-xs font-semibold text-red border border-red/30 px-3 py-1.5 rounded-full hover:bg-red/10 ml-auto"
                      >
