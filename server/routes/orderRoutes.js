...
const { sendOrderConfirmationEmail, sendStatusUpdateEmail, sendDailySalesReport } = require('../config/email');
const { addClient, removeClient, notifyNewOrder } = require('../config/notifyStream');
...
router.post('/', async (req, res) => {
  try {
    const {
      customerName, phone, email, address, items, subtotal, total, couponCode,
      paymentMethod, paymentDetails, orderType, tableNumber, pickupTime, branch, notes, loyaltyRedeem,
    } = req.body;
    ...
    if (type === 'takeaway') effectiveAddress = pickupTime ? `Takeaway — Pickup at ${pickupTime}` : 'Takeaway';
    if (type === 'dine-in') effectiveAddress = tableNumber ? `Dine-In — Table ${tableNumber}` : 'Dine-In';
    ...
    if (couponCode) {
      ...
      if (coupon && coupon.usedCount < coupon.maxUses) {
        coupon.usedCount += 1;
        await coupon.save();
        discount = coupon.type === 'percent' ? Math.round((subtotal || total) * coupon.value / 100) : coupon.value;
      }
    }
    ...
    const order = new Order({
      customerName, phone, email: email || '', address: effectiveAddress,
      items, subtotal: subtotal || total, discount, loyaltyDiscount,
      total, couponCode: couponCode || '',
      loyaltyPointsEarned: pointsEarned,
      paymentMethod: paymentMethod || 'cod',
      paymentDetails: paymentDetails || {},
      orderType: type, tableNumber: tableNumber || '', pickupTime: pickupTime || '',
      branch: branch || 'cantt', notes: notes || '',
    });
    await order.save();

    sendOrderConfirmationEmail(order).catch((e) => console.error('Email:', e.message));
...
router.get('/stats', requireAdmin, async (req, res) => {
  try {
    ...
    const counts = await Promise.all(statuses.map((s) => Order.countDocuments({ status: s, ...branchFilter })));
    const stats = {};
    ...
    stats.weeklyChart = await Order.aggregate([
      { $match: { status: 'delivered', createdAt: { $gte: sevenAgo }, ...branchFilter } },
...
router.get('/daily-report', requireAdmin, async (req, res) => {
  try {
    ...
    const itemMap = {};
    delivered.forEach((o) => o.items.forEach((i) => { itemMap[i.name] = (itemMap[i.name] || 0) + i.qty; }));
  ...
  try {
    ...
    if (/^[0-9a-f]{24}$/.test(raw)) {
      ...
      order = await Order.findOne({
        $expr: { $eq: [{ $substrCP: [{ $toString: '$_id' }, 24 - raw.length, raw.length] }, raw] },
      });
    }
...
router.put('/:id/status', requireAdmin, async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ message: 'Not found' });
    sendStatusUpdateEmail(order).catch((e) => console.error('Email:', e.message));
    res.json(order);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
router.put('/:id/verify-payment', requireAdmin, async (req, res) => {
  try {
    ...
    if (transactionId) order.paymentDetails = { ...order.paymentDetails, transactionId };
    await order.save();
    res.json(order);
  } catch (err) {
    res.status(400).json({ message: err.message });
...
router.put('/:id/kitchen-notes', requireAdmin, async (req, res) => {
  try {
    const order = await Order.findByIdAndUpdate(req.params.id, { kitchenNotes: req.body.kitchenNotes }, { new: true });
    if (!order) return res.status(404).json({ message: 'Not found' });
    res.json(order);
  } catch (err) {
    res.status(400).json({ message: err.message });
